...
$(document).ready(function () {
	...
	// 商品紹介 3階建て/2階建て切り替え
	// $('[name="floorsNumSelect"]:radio').change(function () {
	// 	// 3階建て
	// 	if ($('[id=floorsNumSelect_three]').prop('checked')) {
	// 		$("#product .sectionContentWrap .thumbnailWrap .image").each(function () {
	// 			$("#product .sectionContentWrap .thumbnailWrap .image").each(function () {
	// 				var imgSrc = $(this).children('img').attr('src').split('/');
	// 				var change03 = $(this).children('img').attr('src').replace(imgSrc[13], '03');
	// 				$(this).children('img').attr('src', change03);
	// 			});

	// 		});
	// 		// 2階建て
	// 	} else if ($('[id=floorsNumSelect_two]').prop('checked')) {
	// 		$("#product .sectionContentWrap .thumbnailWrap .image").each(function () {
	// 			$("#product .sectionContentWrap .thumbnailWrap .image").each(function () {
	// 				var imgSrc = $(this).children('img').attr('src').split('/');
	// 				var change02 = $(this).children('img').attr('src').replace(imgSrc[13], '02');
	// 				$(this).children('img').attr('src', change02);
	// 			});
	// 		});
	// 	}
	...
	// });
	$(".floorsNumTwo").css("display", "none");
	$('[name="floorsNumSelect"]:radio').change(function () {
		if ($('[id=floorsNumSelect_three]').prop('checked')) {
			$(".floorsNumThree").css("display", "block");
			$(".floorsNumTwo").css("display", "none");
		} else if ($('[id=floorsNumSelect_two]').prop('checked')) {
			$(".floorsNumThree").css("display", "none");
			$(".floorsNumTwo").css("display", "block");
		}
	...
	$('[name="outerWallSelect"]:radio').change(function () {
		if ($('[id=outerWallSelect_bl]').prop('checked')) {
			$("#outerWall_bl").css("visibility", "visible");
			$("#outerWall_wh").css("visibility", "hidden");
		} else if ($('[id=outerWallSelect_wh]').prop('checked')) {
			$("#outerWall_bl").css("visibility", "hidden");
