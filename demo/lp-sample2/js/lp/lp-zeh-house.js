...
$(document).ready(function () {
	...
	// 商品紹介 外壁の色選択でサムネ画像変更
	$("#outerWall_nv").css("visibility", "hidden");
	$('[name="outerWallSelect"]:radio').change(function () {
		if ($('[id=outerWallSelect_gr]').prop('checked')) {
			$("#outerWall_gr").css("visibility", "visible");
			$("#outerWall_nv").css("visibility", "hidden");
		} else if ($('[id=outerWallSelect_nv]').prop('checked')) {
			$("#outerWall_gr").css("visibility", "hidden");
			$("#outerWall_nv").css("visibility", "visible");
		}
